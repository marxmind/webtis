...
public class License {
	...
	public static String licenseFile(Module module){
		 ...
		 * builderFactory.newDocumentBuilder(); // Document doc =
		 * documentBuilder.parse(xmlFile); // // /////////////normalize //
		 * doc.getDocumentElement().normalize(); //
		 * //System.out.println("Reading conf......"); // // NodeList ls =
		 * doc.getElementsByTagName("module"); // int size=ls.getLength(); // // for(int
		 * i=0; i<size; i++){ // Node n = ls.item(i); // // // if(n.getNodeType() ==
		 * Node.ELEMENT_NODE){ // Element e = (Element)n; // result =
		 * e.getElementsByTagName(module.getName()).item(0).getTextContent(); // i=size;
	...
	public static void main(String[] args) {
		...
		SAXReader reader = new SAXReader();
		Document document = reader.read(xmlFile);
		Node node = document.selectSingleNode("/license/module/"+Module.CHECK_WRITING.getName());
			
		System.out.println(node.getStringValue());
		node.setText("02r03i-03k01a-02r00m01a09a");
		...
		//String result = node.selectSingleNode(module.getName()).getText();
		
		//License lic = new License();
		//boolean isActivated = lic.activateLicenseCode(Module.CHECK_WRITING, "markitalia");
		//boolean isModule = License.checkLicenseExpiration(Module.CHECK_WRITING);
		/*if(isActivated){
			System.out.println("active..");
		}*/
		
		/*
		 * if(isModule){ System.out.println("module expired"); }
		 */
	} 
